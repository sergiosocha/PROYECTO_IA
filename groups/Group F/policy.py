...
class HumbleButHonest(): # Disabled
    def mount(self) -> None:
        self.q_vals = {}
        self.q_counts = {}
    ...
    def actualiza_q(self, reward: float):
        if self.last_action is not None and self.last_state is not None:
            ...
            qval_key = get_qval_codificado(s, a)
            if qval_key in self.q_counts:
                self.q_counts[qval_key] += 1
            else:
                self.q_counts[qval_key] = 1
            n = self.q_counts[qval_key]

            if qval_key not in self.q_vals:
                self.q_vals[qval_key] = 0.0
            old_q = self.q_vals[qval_key]
            new_q = old_q + (1 / n) * (reward - old_q)
            self.q_vals[qval_key] = new_q
    ...
    def act(self, s: np.ndarray) -> int:
        ...
        for col in cols_disponibles:
            scores[col] += self.q_vals.get(col, 0)
